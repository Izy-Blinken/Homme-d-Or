...
// if broadcast, send notif sa lahat
if ($voucher_type === 'broadcast') {

    $label = $discount_type === 'percent' ? $discount_value . '% off' : '₱' . number_format($discount_value, 2) . ' off';

    $expires_label = date('M d, Y', strtotime($expires_at));
    $msg = mysqli_real_escape_string($conn, "Limited voucher available: $code — $label. Valid until $expires_label. First $usage_limit use(s) only!");
