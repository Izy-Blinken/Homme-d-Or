<?php
    ...
    checkAnyPermission($conn, ['can_add_product', 'can_edit_product', 'can_delete_product']);

    $can_add = hasPermission($conn, 'can_add_product');
    $can_edit = hasPermission($conn, 'can_edit_product');
    ...
    if ($filter_sort === 'price-asc') {
        $sort_sql = "p.price ASC";
    }

    if ($filter_sort === 'price-desc') {
        $sort_sql = "p.price DESC";
    }

    if ($filter_sort === 'name-asc') {
        $sort_sql = "p.product_name ASC";
    }

    if ($filter_sort === 'stock-asc') {
        $sort_sql = "p.stock_qty ASC";
    }

    $products = mysqli_query($conn,
    "SELECT p.*, c.category_name, (SELECT image_url FROM product_images WHERE product_id = p.product_id AND is_primary = 1 LIMIT 1) AS primary_image
     FROM products p
            ...
            <main class="container">
                ...
                <form method="GET" action="">
                    ...
                    <div class="filter-bar">
                        ...
                        <a href="productManagement.php" id="reset-link" class="reset-btn" style="text-decoration: none; color:black;">Reset</a>
                    </div>
